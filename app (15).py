...
def prepare_report_data(
    ...
    *,
    ...
    """Prepare the report data for the selected production date or month.

    If ``by_month`` is False (default), the function filters ``time_df`` to include
    only rows where the ``Production Date`` matches ``selected_date``.  When
    ``by_month`` is True, rows are included when their ``Production Date`` falls
    within the same calendar month and year as ``selected_date``.  This allows
    aggregating work orders across an entire month while preserving the
    existing grouping and display logic.

    Returns a dictionary with keys:
      - ``groups``: a list of tuples (craft_name, payload) where payload contains a
        'detail' DataFrame with the selected columns for that craft.
    ...
    """
    # Filter the time_df based on the view mode.  When by_month=True, match on
    # year and month rather than an exact date.  Convert values to datetime for
    # reliable month/year comparisons, coercing invalid entries to NaT.
    if by_month:
        try:
            target_dt = pd.to_datetime(selected_date)
        except Exception:
            target_dt = pd.to_datetime(str(selected_date), errors="coerce")
        dt_prod_date = pd.to_datetime(time_df["Production Date"], errors="coerce")
        mask = (dt_prod_date.dt.month == target_dt.month) & (dt_prod_date.dt.year == target_dt.year)
        f = time_df[mask].copy()
    else:
        f = time_df[time_df["Production Date"] == selected_date].copy()

    # Normalize AddressBookNumber in both frames for joining
    ...
    # Merge with address book to get craft and name information.  Use left
    # join so rows without a match are retained and will be assigned to
    # "Unassigned" later.
    merged = f.merge(
    ...
    # Prefer the name from the time dataframe if present; otherwise use the
    # address book name.
    merged["Name"] = merged["Name"].fillna(merged["AB_Name"])
    merged = merged.drop(columns=["AB_Name"])
    # Identify unmapped people (those without a craft assignment).
    ...
    # Assign unmapped rows to "Unassigned" craft
...
# log of work orders.
# Configure the Streamlit page.  The title reflects that the app can display
# either daily or monthly work orders, so we avoid hard-coding "Daily" in
# the page title.  The layout remains wide for maximum horizontal space.
st.set_page_config(page_title="Work Order Log", layout="wide")
st.title("Work Order Log")
...
# Determine available dates and months from the Production Date column.  We
# coerce to datetime to handle any string-based dates and drop missing
# values.  Unique dates are presented for daily mode and unique months
# (year-month) are presented for monthly mode.
dt_prod_dates = pd.to_datetime(time_df["Production Date"], errors="coerce")
unique_dates = sorted(dt_prod_dates.dropna().dt.date.unique())
unique_months = sorted(dt_prod_dates.dropna().dt.to_period("M").unique())

# Provide a view mode selector so the user can choose between viewing a
# single day or the entire month.  Default to daily view.
view_mode = st.radio(
    "View Mode",
...
if view_mode == "Daily":
    # Build a mapping from display labels to date objects for the date
    # selector.  Labels are formatted as MM/DD/YYYY.
    date_labels = [datetime.strftime(pd.to_datetime(d), "%m/%d/%Y") for d in unique_dates]
    label_to_date = dict(zip(date_labels, unique_dates))
    ...
    # Build a mapping from display labels to period objects for the month
    # selector.  Use full month name and year (e.g., 'January 2023').  Convert
    # each Period to its corresponding timestamp (first day of the month)
    # when passing to prepare_report_data.
    month_labels = [period.to_timestamp().strftime("%B %Y") for period in unique_months]
    label_to_period = dict(zip(month_labels, unique_months))
    ...
    selected_period = label_to_period[selected_month_label]
    # Use the first day of the selected period as the representative date.
    selected_date = selected_period.to_timestamp().date()
    by_month_flag = True
    display_label = selected_month_label

# After determining the view mode and selected date, present a third selector
# allowing the user to choose which craft to display.  The options come
# directly from the craft order configuration.  An "All Crafts" option
# restores the original behavior of showing every craft.
craft_options = craft_df["Craft Description"].tolist()
craft_select_options = ["All Crafts"] + craft_options
...
report = prepare_report_data(
    ...
    craft_df,
    selected_date,
    by_month=by_month_flag,
)

# -----------------------------------------------------------------------------
# Craft filtering
#
# Now that the report has been generated, filter it based on the selected
# craft.  If "All Crafts" is selected, retain all groups.  Otherwise,
# keep only the matching craft and rebuild the full_detail accordingly.  This
# ensures that downstream loops and the PDF builder reference the filtered
# report.
if selected_craft == "All Crafts":
    filtered_report = report
else:
    # Filter groups to include only the selected craft.  Rebuild the
    # full_detail DataFrame accordingly.  Unmapped people list is preserved.
    filtered_groups = [g for g in report["groups"] if g[0] == selected_craft]
    if filtered_groups:
        # Concatenate detail DataFrames for the selected craft.  Since there is
        # only one craft, simply take the first payload.  Use concat for future
        # extensibility.
        detail_frames = [payload["detail"] for _, payload in filtered_groups]
        filtered_full_detail = pd.concat(detail_frames, ignore_index=True)
    else:
        filtered_full_detail = pd.DataFrame(columns=DISPLAY_COLUMNS)
    filtered_report = {
        "groups": filtered_groups,
        "full_detail": filtered_full_detail,
        "unmapped_people": report.get("unmapped_people", []),
    }
...
for craft_name, payload in filtered_report["groups"]:
...
pdf_bytes = build_pdf(filtered_report, display_label, sort_option)
